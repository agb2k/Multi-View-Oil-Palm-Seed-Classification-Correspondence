...
for (top,bottom)  in seed_src_boxes:
    ...
    for j in range(len(src_pts)):
        ...
        if src_list[j][0][0]>top[0] and src_list[j][0][0]<bottom[0]:
            print("check")
            if src_list[j][0][1]>top[1] and src_list[j][0][1]<bottom[1]:
                print("check 2")
                correct_points= j
                src_box = (top,bottom)
    ...
    if src_box:
        ...
        for (top,bottom)  in seed_dst_boxes:
            dst_box = []
            if dst_list[correct_points][0][0]>top[0] and dst_list[correct_points][0][0]<bottom[0]:
                print("check")
                if dst_list[correct_points][0][1]>top[1] and dst_list[correct_points][0][1]<bottom[1]:
                    print("check 2")
