...
def find_correspondence(path_ref, path_trgt,path_save,trgt):
    ...
    #csv output
    csv_array.append([len(keypoints_src),len(keypoints_dst),len(matches), len(src_list), (len(src_list)/len(matches))*100])
...
os.chdir(r"F:/CompV/Multiview_jpg/Good seeds")
CSV_SEED_INDIVIDUAL_HEADER = ["Keypoints1", "keypoints2", "Matches", "Inliers", "% of Inliers"]


outpath = r"F:/CompV/Multiview_jpg/Good seeds" 
outcsv = os.path.join(outpath, 'Good Seeds Inliers.csv')
...
outcsv = os.path.join(outpath, 'Bad Seeds Inliers.csv')
if not os.path.exists(outcsv):
    with open(outcsv, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(CSV_SEED_INDIVIDUAL_HEADER)
